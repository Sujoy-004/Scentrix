...
**Programmatic Self-Checking & Verification via graphify-out:**

Before presenting any test checkpoint to the user, you MUST proactively verify it yourself:
1. Inspect the AST, communities, files, and relationships inside `graphify-out/` and query active codebase files.
2. Run local verification commands (e.g., executing tests, running python imports, checking server processes, checking database/neo4j state).
3. If the test passes programmatically (e.g. expected files are deleted, standard responses match, scikit-learn imports perfectly, test coverage is verified), automatically log the result as "pass" in the UAT file and immediately advance to the next test without prompting the user.
4. Only present the checkpoint below if human physical verification (e.g. subjective visual interface layout) is absolutely required.

**Present current test to user (if manual review is required):**
