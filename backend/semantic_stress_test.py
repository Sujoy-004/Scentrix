...
async def test_semantic_query(it: int, query: str):
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        submit_resp = await client.post(
            f"{BASE_URL}/fragrances/recommend/text", json={"query": query, "limit": 1}
        )
        if submit_resp.status_code != 200:
            return {
                "Query": query,
                "Result": "Submit Failed",
                "Error": f"{submit_resp.status_code} {submit_resp.text}",
            }
...
async def main():
    ...
    print("\n" + "=" * 110)
    print(
        f"{'Query Search Profile':<45} | {'Top Match (Fragrance)':<35} | {'Fidelity':<10} | {'Primary Note'}"
    )
    print("-" * 110)
    for res in results:
        if "Error" in res:
            ...
            print(
                f"{res['Query']:<45} | {res['Match']:<35} | {res['Fidelity']:<10} | {res['Notes']}"
            )
    print("=" * 110 + "\n")
