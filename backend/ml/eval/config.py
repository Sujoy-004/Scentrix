...
class EvalConfig(BaseSettings):
    ...
    gt_mode: str = Field(
        default="jaccard",
        pattern="^(jaccard|brand_accord)$",
        description="Ground truth relevance mode: 'jaccard' (same accord + Jaccard notes > threshold) or 'brand_accord' (same brand + same accord).",
    )
    evaluation_mode: str = Field(default="pure_cold", pattern="^(pure_cold|quiz_init|warm_ref|quiz_sensitivity|stratification)$")
    # PIPE-03 contract: quiz_length is a quiz length k in {1,3,5,7,10} (max 10)
