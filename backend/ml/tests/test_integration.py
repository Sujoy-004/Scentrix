...
try:
    ...
    # Legacy Neo4j ingestion pipeline (app.services.graph / ml.pipeline)
    # was superseded by the standalone training pipeline (backend/train.py).
    # The CLI below still works when those modules exist; pytest collection
    # no longer hard-fails on their absence.
    init_neo4j = None
    close_neo4j = None
    FragranceDataCleaner = None
    FragranceGraphIngestor = None
