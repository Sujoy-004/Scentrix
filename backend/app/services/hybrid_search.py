...
class HybridRecommender:
    ...
    def __init__(self):
        ...
        self.vector_index = self.pc.Index("scentscape-descriptions") if self.pc else None
    ...
    def _query_vector_dna(self, user_vec: List[float], limit: int = 100) -> List[Dict[str, Any]]:
        ...
        if self.vector_index:
            try:
                ...
                return [{"id": m["id"], "score": m["score"], "metadata": m.get("metadata", {})} for m in res["matches"]]
            except Exception as e:
                logger.warning(f"Pinecone query failed, attempting local fallback: {e}")

        # 2. Local Semantic Brain Fallback (High-Fidelity)
        from app.routers.recommendations import _catalog_embeddings_cache, load_recommendation_catalog
    ...
    def _rerank_genetic_match(self, candidate_ids: List[str], user_rated_ids: List[str]) -> List[Dict[str, Any]]:
        """Phase 2: High-precision graph reranking."""
        with self.driver.session() as session:
            ...
            MATCH (rec)-[:HAS_NOTE]->(n:Note)<-[:HAS_NOTE]-(seed)
    ...
    def get_recommendations(self, user_profile_vec: List[float], user_seed_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        # Step 1: Candidate Selection (Vector)
        ...
        # Step 3: Reciprocal Rank Fusion (RRF) / Scalar Fusion
        refined = []
        for c in candidates:
            c_id = c["id"]
