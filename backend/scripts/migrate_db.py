...
async def migrate():
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE fragrance_ratings ADD COLUMN IF NOT EXISTS quiz_rating FLOAT"))
        
        # Make perceptual dims nullable if they aren't already
        for col in ["rating_sweetness", "rating_woodiness", "rating_longevity", "rating_projection", "rating_freshness", "overall_satisfaction"]:
            await conn.execute(text(f"ALTER TABLE fragrance_ratings ALTER COLUMN {col} DROP NOT NULL"))
