...
async def run_simulation(it_name, rating_logic):
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if reg_resp.status_code not in (200, 201):
            return {
                "Iteration": it_name,
                "Error": f"Reg Failed: {reg_resp.status_code} {reg_resp.text}",
            }
        ...
        eval_resp = await client.post(
            f"{BASE_URL}/quiz/session/{session_id}/evaluate", json={"force": False}, headers=headers
        )
...
async def main():
    ...
    for r in results:
        if "Error" in r:
            ...
            continue
        print(
            f"{r['Iteration']:<28} | {r['Conf. Score']:<6} | {r['Fidelity']:<8} | {r['Band']:<8} | {str(r['Ext. Required'])[0]:<5} | {r['Stop Reason']}"
        )
