...
async def run_simulation(it_num: int, profile_name: str, rating_strategy: str):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        reg_resp = await client.post(
            ...
            json={"email": email, "password": password, "full_name": profile_name},
        )
        if reg_resp.status_code not in (200, 201):
            return {
                "Iteration": it_name,
                "Strategy": rating_strategy,
                "Error": f"Reg Failed: {reg_resp.text}",
            }
        ...
        if init_resp.status_code != 200:
            return {
                "Iteration": it_name,
                "Strategy": rating_strategy,
                "Error": f"Quiz Start Failed: {init_resp.status_code} {init_resp.text}",
            }
        ...
        for i, q in enumerate(questions):
            ...
            await client.post(
                f"{BASE_URL}/quiz/session/{session_id}/responses",
                headers=headers,
        ...
        eval_resp = await client.post(
            f"{BASE_URL}/quiz/session/{session_id}/evaluate", headers=headers
        )
...
async def main():
    strategies = [
        (1, "The Aristocrat", "Max"),
        (2, "The Critic", "Min"),
        (3, "The Chaos", "Random"),
        (4, "The Balanced", "Gaussian"),
        (5, "The Bipolar", "Polar"),
        (6, "The Evolver", "Gradual"),
        (7, "The Consistent", "Gaussian"),
        (8, "The Skeptic", "Min"),
        (9, "The Enthusiast", "Max"),
        (10, "The Noise", "Random"),
        (11, "The Anchor", "Gaussian"),
        (12, "The Switcher", "Polar"),
        (13, "The Ascender", "Gradual"),
        (14, "The Descender", "Min"),
        (15, "The Peak", "Max"),
        (16, "The Valley", "Min"),
        (17, "The Plateau", "Gaussian"),
        (18, "The Pulse", "Polar"),
        (19, "The Drift", "Gradual"),
        (20, "The Vector", "Random"),
    ]
    ...
    print("\n" + "=" * 80)
    print(
        f"{'Iteration':<30} | {'Strategy':<10} | {'Conf':<5} | {'Band':<6} | {'Fidelity':<8} | {'Reason'}"
    )
    print("-" * 80)
    for res in results:
        if "Error" in res:
            ...
            print(
                f"{res['Iteration']:<30} | {res['Strategy']:<10} | {conf:<5} | {band:<6} | {res['Fidelity']:<8} | {res['Reason']}"
            )
    print("=" * 80 + "\n")
